...
def generate_test_data_file(employees: List[Dict[str, str]], output_file: str = "enron_test_data.py"):
    """生成可导入的Python测试数据文件"""
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for emp in employees:
            f.write(f"    ('{emp['username']}', '{emp['password']}', '{emp['email']}'),\n")
...
def generate_sql_insert_file(employees: List[Dict[str, str]], output_file: str = "enron_test_data.sql"):
    """生成SQL INSERT语句文件"""
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for i, emp in enumerate(employees):
            ...
            f.write(f"('{emp['username']}', '{emp['password']}', '{emp['email']}'){comma}\n")
        ...
        for i, emp in enumerate(employees):
            comma = ',' if i < len(employees) - 1 else ';'
            f.write(f"('{emp['username']}', pgp_sym_encrypt('{emp['password']}', 'my-secret-key'), '{emp['email']}'){comma}\n")
