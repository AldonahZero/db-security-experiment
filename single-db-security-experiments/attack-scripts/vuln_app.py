...
pg_conn = psycopg2.connect(
    host="postgres-db", dbname="juiceshop_db", user="youruser", password="password123"
)
...
def pg_users():
    ...
    return jsonify({"query": query, "rows": rows, "elapsed_ms": duration_ms})
...
def mongo_login():
    ...
    duration_ms = (time.time() - start) * 1000
    return jsonify({"query": query, "results": docs, "elapsed_ms": duration_ms})
