...
在 Leader CPU 压力组中，扰动期成功吞吐量相对正常期下降 44.84%，成功请求 P99 延迟由 358.69 ms 上升到 712.25 ms，恢复到正常期 90% 吞吐且成功请求 P99 不高于正常期 110% 的时间为 4.58 s。网络扰动组的成功吞吐量下降 97.09%，扰动期失败率为 98.55%，恢复时间为 6.01 s。应用侧限流组在相同 CPU 压力下注入下的成功吞吐量下降为 33.57%，扰动期成功请求 P99 延迟为 79.87 ms，说明限流能够降低部分尾延迟压力，但会以主动压低吞吐作为代价。扰动期 TiKV 峰值 CPU 最高节点为 TiKV节点1，峰值为 206.18%。
