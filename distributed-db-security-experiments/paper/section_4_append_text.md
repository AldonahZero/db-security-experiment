...
PostgreSQL+Citus 对照使用 1 个协调节点与 3 个工作节点，通过 Citus 扩展将 `citus_items` 和 `citus_events` 按 `item_id` 分布式分片。在 Citus 原生分布式执行路径下，均匀流量 P99 延迟为 1163.79 ms，90% 热点键流量 P99 延迟为 920.16 ms，吞吐量为 717.78 请求/秒，失败率为 19.82%。90% 热点流量后观测到 32 个 Citus 分片，热点键所在工作节点为 Citus工作节点0。热点期间 PostgreSQL+Citus 峰值 CPU 最高节点为 Citus工作节点0，峰值为 90.43%。
